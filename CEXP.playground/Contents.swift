...
var realSave:[Double]=[]
var imaginarySave:[Double]=[]

for n in (0...10){
    angle=Double(n)*2.0*pi/10.0
    HW.r=0.0
    HW.j=angle
    let out=CEXP(c1: HW)
    real=out.r
    realSave.append(real)
 
    imaginary=out.j
    
    imaginarySave.append(imaginary)
}
    print("real part of out=\(realSave)")
    print("\nimaginary part of out=\(imaginarySave)")
